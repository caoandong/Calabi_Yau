...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            #correct = tf.equal(tf.argmax(pred, 1), labels_pl)
            correct = tf.squared_difference(pred, labels_pl)
        ...
        output_test_path = 'log/test/loss_test.txt'
        output_test = open(output_test_path, 'a')
        output_train_path = 'log/train/loss_train.txt'
        output_train = open(output_train_path, 'a')

        for epoch in range(MAX_EPOCH):
            log_string('**** EPOCH %03d ****' % (epoch))
            sys.stdout.flush()

            train_one_epoch(sess, ops, train_writer, output_train)
            eval_one_epoch(sess, ops, test_writer, output_test)
...
def train_one_epoch(sess, ops, train_writer, output_train):
    ...
    for fn in range(len(TRAIN_FILES)):
        ...
        current_data = provider.fix_data_size(current_data, NUM_POINT)
        current_label = provider.invert_label(current_label)
        ...
        #current_data = np.array(current_data) #fix_data_size() already turned current_data into a numpy array
        #current_label = np.array(current_label) #invert_label() already turned current_label into a numpy array
        ...
        print('pred: ', pred_val)
        print('correct: ', current_label[start_idx:end_idx])
        #log_string('pred: %f' % pred_val)
        #log_string('correct: %f' % current_label)
        mean_loss = loss_sum / float(num_batches)
        log_string('mean loss: %f' % (loss_sum / float(num_batches)))
        log_string('accuracy: %f' % (total_correct / float(total_seen)))

        # Save the loss values to a .txt file
        output_train.write("%s\n" % mean_loss)


def eval_one_epoch(sess, ops, test_writer, output_test):
    """ ops: dict mapping from string to tf ops """
    is_training = False
    total_correct = 0
    total_seen = 0
