...
def mat_mul(param, batch_size, num_param):
    with tf.variable_scope("mat_mul", reuse=True):
        ...
        #input_param_2 = tf.square(input_param_1, name="input_param_2")
        for row in range(batch_size):
            ...
            for i in range(num_param):
                ...
                param_tmp = tf.concat([param_tmp,param_tmp_2], 1)
            try:
                input_parameter = tf.stack([input_paramter, param_tmp])
            except:
        ...
        # Output: (batch_size, 1)
        W1 = tf.Variable(tf.random_normal([num_param*2, 1], dtype=tf.float32), name="W")
